...
export default function RentVsBuy() {
  ...
  const mortgage = monthlyRate > 0
    ...
    : loan / n
  const monthlyOwn = mortgage + homePrice * 0.012 / 12 + 150
  ...
  const fmtDiff = diff.toLocaleString('en-US', { maximumFractionDigits: 0 })

  return (
    <div style={styles.card}>
      <h3 style={styles.title}>{t('rentVsBuy.title')}</h3>
      <p style={styles.desc}>{t('rentVsBuy.desc')}</p>

      <div style={styles.fields}>
        <label style={styles.label}>{t('rentVsBuy.monthlyRent')}<input type="number" value={rent} onChange={(e) => setRent(+e.target.value)} style={styles.input} /></label>
        <label style={styles.label}>{t('rentVsBuy.homePrice')}<input type="number" value={homePrice} onChange={(e) => setHomePrice(+e.target.value)} style={styles.input} /></label>
        <label style={styles.label}>{t('rentVsBuy.downPayment')}<input type="number" value={downPct} onChange={(e) => setDownPct(+e.target.value)} style={styles.input} /></label>
        <label style={styles.label}>{t('rentVsBuy.interestRate')}<input type="number" value={rate} onChange={(e) => setRate(+e.target.value)} style={styles.input} step={0.1} /></label>
        <label style={styles.label}>{t('rentVsBuy.compareYears')}<input type="number" value={years} onChange={(e) => setYears(+e.target.value)} style={styles.input} /></label>
        <label style={styles.label}>{t('rentVsBuy.annualRentIncrease')}<input type="number" value={rentIncrease} onChange={(e) => setRentIncrease(+e.target.value)} style={styles.input} /></label>
      </div>

      <div style={styles.comparison}>
        <div style={{ ...styles.colCard, borderColor: better === 'rent' ? '#6366f1' : 'rgba(255,255,255,0.1)' }}>
          <span style={styles.colTitle}>{t('rentVsBuy.renting')}</span>
          <span style={styles.colValue}>${totalRent.toLocaleString('en-US', { maximumFractionDigits: 0 })}</span>
          <span style={styles.colSub}>{t('rentVsBuy.totalOver')} {years} {t('rentVsBuy.years')}</span>
          <span style={styles.colDetail}>{t('rentVsBuy.equityBuilt')}: $0</span>
        </div>
        <div style={styles.vs}>VS</div>
        <div style={{ ...styles.colCard, borderColor: better === 'buy' ? '#10b981' : 'rgba(255,255,255,0.1)' }}>
          <span style={styles.colTitle}>{t('rentVsBuy.buying')}</span>
          <span style={styles.colValue}>${totalOwn.toLocaleString('en-US', { maximumFractionDigits: 0 })}</span>
          <span style={styles.colSub}>{t('rentVsBuy.totalOver')} {years} {t('rentVsBuy.years')}</span>
          <span style={styles.colDetail}>{t('rentVsBuy.estEquity')}: ${equity.toLocaleString('en-US', { maximumFractionDigits: 0 })}</span>
        </div>
      </div>

      <div style={styles.verdict}>
        {better === 'buy'
          ? `${t('rentVsBuy.buying')} saves ~$${fmtDiff} (${years} ${t('rentVsBuy.years')})`
          : `${t('rentVsBuy.renting')} saves ~$${fmtDiff} (${years} ${t('rentVsBuy.years')})`
        }
      </div>
    </div>
  )
...
const styles = {
  ...
  comparison: { display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem', flexWrap: 'wrap', justifyContent: 'center' },
  colCard: { flex: '1 1 180px', textAlign: 'center', padding: '1.2rem', borderRadius: '12px', border: '2px solid', background: 'rgba(255,255,255,0.02)', display: 'flex', flexDirection: 'column', gap: '0.3rem' },
